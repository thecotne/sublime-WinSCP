import sublime_plugin, sublime, os, sys, re, subprocess, json
...
class browse_with_winscpCommand(sublime_plugin.WindowCommand):
	def run(self, edit = None):
		...
		if hasActiveView() is False:
			file_path = os.path.dirname(guessConfigFile(sublime.active_window().folders(), configName))
		else:
			file_path = os.path.dirname(sublime.active_window().active_view().file_name())

		_file = getConfigFile(file_path, configName)

		if isString(_file):
			conf = parseJson(_file)
			subprocess.Popen(
				'"C:\Program Files\WinSCP\WinSCP.exe" '+
				conf.get('type')+'://'+conf.get('user')+':'+conf.get('password')+'@'+conf.get('host')+':'+conf.get('port')+
				'"'+conf.get('remote_path')+'"')
